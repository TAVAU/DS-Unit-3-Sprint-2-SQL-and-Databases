...
# get_armorys = "SELECT * FROM armory_item;"
# armorys = sl_curs.execute(get_armorys).fetchall()

# for armory in armorys:
#   print(armory[1])
#   armory_document = {
#       'name': armory[1],
#       'value': armory[2],
#       'weight': armory[3]
#   }
#   collection.insert_one(armory_document)

# insert armory_weapon
# get_armory_weapons = """
# SELECT name, power FROM armory_item i
# JOIN armory_weapon w
# WHERE i.item_id = w.item_ptr_id;
# """
# armory_weapons = sl_curs.execute(get_armory_weapons).fetchall()

# for armory_weapon in armory_weapons:
#   print(armory_weapon)
#   query = { "name": armory_weapon[0] }
#   newvalues = { "$set": { "weapon": { "power": armory_weapon[1] } } }
#   collection.update_one(query, newvalues)

pprint(len(list(collection.find({'weapon': {"$exists": True}}))))